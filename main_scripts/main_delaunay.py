...
from models.set_to_graph_mlp import SetToGraphMLP
from models.set_to_graph_gnn import SetToGraphGNN
from models.set_to_graph_triplets import SetToGraphTri
...
def main():
    ...
    if many_sizes:
        ...
        train_data = get_delaunay_loader_many_sizes(config, train=True)#, device=device)
        print('Generating validation data, n in 20-80...', flush=True)
        val_data = get_delaunay_loader_many_sizes(config, train=False)#, device=device)
    else:
        print('Generating training data, n=50...', flush=True)
        train_data = get_delaunay_loader(config, train=True)  # , device=device)
        print('Generating validation data, n=50...', flush=True)
        val_data = get_delaunay_loader(config, train=False)  # , device=device)
    ...
    if config.baseline == 'mlp':
        ...
        model = SetToGraphSiam(2,
                               [700, 700, 700, 1400, 700, 700, 112],
    ...
    for epoch in range(1, config.epochs + 1):
        train_info = train_epoch(train_data, epoch, model, optimizer, device)
        train_loss[epoch - 1], train_acc[epoch - 1], train_precision[epoch - 1], train_recall[epoch - 1], \
        train_f1[epoch - 1] = train_info['loss'], train_info['acc'], train_info['precision'], train_info['recall'],\
                              train_info['f1']
        with torch.no_grad():
